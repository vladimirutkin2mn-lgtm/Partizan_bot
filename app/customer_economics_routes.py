...
from app.distribution_analytics_schemas import CustomerDistributionEconomicsView
from app.distribution_analytics_service import distribution_analytics_service

router = APIRouter(tags=["customer-economics"])
...
@router.get(
    "/customer/workspace/{project_id}/distribution-economics",
    response_model=CustomerDistributionEconomicsView,
)
def get_customer_distribution_economics(
    project_id: UUID,
    ...
    if not product_id:
        raise HTTPException(status_code=409, detail="Customer project has no confirmed product")
    try:
        return distribution_analytics_service.customer_economics(UUID(str(product_id)))
