...
TIKTOK_PAID_CONTROL_NAMESPACE = "tiktok_paid_control_snapshot"


class TikTokPaidControlSnapshotView(BaseModel):
    action_id: UUID
    experiment_id: UUID
    product_id: UUID
    campaign_id: str
...
class TikTokPaidControlService:
    def __init__(
        self,
        *,
    ...
    def sync(self, action_id: UUID) -> TikTokPaidControlSnapshotView:
        context = self._context(action_id)
        previous = self.get(action_id)
        now = datetime.now(UTC)
        try:
            state = self._client.get_campaign_state(
                connection=context.connection,
                access_token=context.access_token,
                campaign_id=context.campaign_id,
            )
            insights = self._client.get_campaign_insights(
                connection=context.connection,
                access_token=context.access_token,
                campaign_id=context.campaign_id,
            )
        ...
        if delta > 0 and context.action_status == DistributionActionStatus.EXECUTED:
            ...
            last_error = "TikTok reported spend before the local paid experiment was RUNNING"
            requires_reconciliation = True

        if spend_regressed:
            last_error = "TikTok cumulative spend is lower than the amount already synced locally"
            requires_reconciliation = True

        budget_guardrail = provider_spend + 0.005 >= context.spec.budget_cap
        unexpected_active = (
            context.action_status != DistributionActionStatus.EXECUTED
            and state.operation_status.upper() != "DISABLE"
        )
        ...
        if budget_guardrail:
            ...
            state.operation_status.upper() == "DISABLE"
        ...
        if state.operation_status.upper() == "DISABLE":
    ...
    def pause(
        ...
        *,
        ...
        snapshot = TikTokPaidControlSnapshotView(
            ...
            budget_cap=context.spec.budget_cap,
            sync_state="SYNCED" if state.operation_status != "UNKNOWN" else "UNKNOWN",
    ...
    def reset(self) -> None:
        if self._store.ephemeral:
            self._store.clear_namespace(TIKTOK_PAID_CONTROL_NAMESPACE)

    def _context(self, action_id: UUID) -> _TikTokControlContext:
        ...
        if (
            ...
            raise ValueError("TikTok paid control supports TIKTOK PAID_CAMPAIGN actions only")
        if action.status not in {
            DistributionActionStatus.APPROVED,
            DistributionActionStatus.EXECUTED,
        }:
            raise ValueError("TikTok paid control requires an APPROVED or EXECUTED action")
        if action.experiment_id is None:
            raise ValueError("TikTok paid action has no DistributionExperiment")
        ...
        if receipt is None or receipt.provider != "tiktok-marketing-api":
            raise ValueError("TikTok paid control requires a TikTok execution receipt")
        if receipt.outcome not in {
            AdapterExecutionOutcome.STAGED,
            AdapterExecutionOutcome.EXECUTED,
        }:
            raise ValueError("TikTok paid control requires a STAGED or EXECUTED receipt")
        provider_ids = receipt.metadata.get("provider_ids")
        if not isinstance(provider_ids, dict) or not provider_ids.get("campaign_id"):
            raise ValueError("TikTok execution receipt has no campaign_id")
        spec = self._spec_service.get(action_id)
        if spec is None:
            raise ValueError("PaidCampaignSpec is required for TikTok paid control")
        connection = self._connection_service.require_active(experiment.product_id)
        if not connection.report_type or not connection.report_data_level:
            raise ValueError(
                "TikTok paid control requires explicit report_type and report_data_level connection config"
            )
    ...
    def _ingest_spend_delta(
        self,
        context: _TikTokControlContext,
        provider_spend: float,
        delta: float,
        insights: TikTokCampaignInsights,
    ) -> None:
        spend_id = uuid5(
            NAMESPACE_URL,
            f"tiktok:{context.campaign_id}:cumulative:{provider_spend:.2f}",
        )
    ...
    def _pause_and_verify(
        self,
        context: _TikTokControlContext,
        current: TikTokCampaignState,
    ) -> tuple[TikTokCampaignState, Literal["CONFIRMED", "UNKNOWN"], str | None]:
        try:
        ...
        if verified.operation_status.upper() != "DISABLE":
            return verified, "UNKNOWN", "TikTok did not confirm operation_status=DISABLE"
        return verified, "CONFIRMED", None

    def _unknown_snapshot(
        self,
        context: _TikTokControlContext,
        previous: TikTokPaidControlSnapshotView | None,
        error: str,
        now: datetime,
    ) -> TikTokPaidControlSnapshotView:
        return TikTokPaidControlSnapshotView(
            action_id=context.action_id,
            experiment_id=context.experiment_id,
            product_id=context.product_id,
            campaign_id=context.campaign_id,
            operation_status=previous.operation_status if previous else "UNKNOWN",
            primary_status=previous.primary_status if previous else None,
            secondary_status=previous.secondary_status if previous else None,
            provider_spend=previous.provider_spend if previous else 0.0,
            synced_spend=previous.synced_spend if previous else 0.0,
            last_spend_delta=0.0,
            impressions=previous.impressions if previous else 0,
    ...
    def _persist(self, snapshot: TikTokPaidControlSnapshotView) -> None:
        self._store.put(
...
tiktok_paid_control_service = TikTokPaidControlService()
