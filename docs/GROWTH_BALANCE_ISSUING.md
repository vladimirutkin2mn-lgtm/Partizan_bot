...
The current implementation does not automate adding a Partizan Issuing card to Meta billing. When Issuing is enabled, use an approved secure Stripe card-detail surface and provider billing UI/process. Never copy PAN/CVC through Partizan APIs or persistence.

After provider-side attachment, record only the non-sensitive binding fact through the protected Partizan operator flow. Do not confirm a binding until the exact customer provider account is verified.
